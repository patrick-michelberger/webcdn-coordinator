...
Server.prototype.getPeerByItem = function(sender, objectHash, callback) {
    ...
    if (list_connections && list_connections.length > 0) {
        ...
        for (var i = 0; i < list_connections.length; i++) {
            var connection = list_connections[i];
            if (connection.peerid != sender.peerid && this.isUploadAllowed(connection.peerid)) {
                var geolocation = connection.nc.geolocation;
        ...
        */
        ...
        if (sender_geolocation && sender_geolocation.lat && sender_geolocation.lon) {
            ...
            this.getNearestPeer(query, function(nearestNeighbor, probability) {
...
Server.prototype.getNearestPeer = function(query, callback) {
    var items = this.peers;
    var fields = [{
        name: "lat",
